...
Every request is interrogated over the wire, so the action always answers with what the endpoint actually advertises rather than echoing the shipped catalog. A draft names the endpoint the form shows; a request naming a route the installed catalog ships without one falls back to that route's installed default endpoint and protocol. A request that names no endpoint at all — a route the catalog does not describe and a draft with no base URL — is told to set one or enter its models by hand.

A draft carries the credential the user typed, if any; a route that already stored one shows a configuration surface only a redacted descriptor, so the interrogation resolves that route's `apiKeyEnv` rather than going out unauthenticated and reporting the endpoint's 401 as a wrong key. A typed key wins, being the one under test. Resolution happens only on the path that reaches the network, so an endpoint this build cannot read costs no credential lookup. A supplied or stored probe key is trimmed and format-checked the same way, so a value no HTTP header can carry is refused immediately as `LlmError('INVALID_CREDENTIAL')` instead of reaching `fetch`, where it would surface as an opaque `ByteString` failure indistinguishable from an unreachable endpoint.
...
Most listings disclose an id and nothing else; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them, entries without a usable id are skipped rather than failing the whole listing, and everything else the adopting surface still owes. A discovered id the installed catalog also describes carries `reasoningEfforts` — `false`, or the offered levels and their wire spellings — since no listing endpoint reports a model's reasoning; `off` offered without a spelling is the one `null` value, and an id the catalog does not cover carries none. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
