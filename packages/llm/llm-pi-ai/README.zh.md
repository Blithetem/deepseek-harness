...
每一次请求都会经协议层询问，因此该动作始终以端点实际公布的模型作答，而不是照搬已安装目录。草稿点名的就是表单当前显示的端点；点名了**已安装 catalog 所提供路由**却没有端点时，会回退到该路由已安装的默认端点与协议。完全没有端点可问的请求——catalog 未描述的路由、草稿又没填 API 地址——则被告知去设置一个或手工填写模型。

草稿携带的是用户当下键入的凭据（如果有）；已经存好凭据的路由，在配置界面上只呈现一个脱敏描述符，因此询问会解析该路由的 `apiKeyEnv`，而不是不带认证发出去、再把端点的 401 报成密钥不对。键入的密钥优先，因为那正是被测试的那一把。解析只发生在真正要联网的路径上，因此本构建无法读取协议的端点不会产生凭据查找。用户提供或已存储的探测密钥也会经过同样的去除空白与格式校验：HTTP 标头无法承载的值会被立即以 `LlmError('INVALID_CREDENTIAL')` 拒绝，而不会传到 `fetch`——否则会呈现为一个和端点不可达难以区分的、语义不明的 `ByteString` 失败。
